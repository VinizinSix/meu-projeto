...
public class LojaGamesGUI extends JFrame {
    ...
    public List<Game> filtrarPorCategoria(String categoria) {
        return catalogo.stream()
                .filter(g -> g.getGenero().equalsIgnoreCase(categoria))
                .toList();
    }
    public List<Game> filtrarPorPreco(BigDecimal limite) {
        return catalogo.stream()
                .filter(g -> g.getPreco().compareTo(limite) <= 0)
                .toList();
    }
